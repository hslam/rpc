...
import (
	...
	"github.com/hslam/rpc/benchmarks/context/service"
...
func main() {
	...
	for i := 0; i < clients; i++ {
		if conn, err := rpc.Dial(network, addr, codec); err != nil {
			...
			wrkClients = append(wrkClients, &WrkClient{
				Conn: conn,
...
func (c *WrkClient) Call() (int64, int64, bool) {
	...
	ctx := c.ctxPool.Get().(context.Context)
	if err := c.Conn.CallWithContext(ctx, "Arith.Multiply", req, &res); err != nil {
		fmt.Println(err)
		return 0, 0, false
	}
	if res.Pro == A*B {
