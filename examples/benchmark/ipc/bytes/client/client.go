...
func main()  {
	fmt.Printf("./client -network=%s -codec=%s -compress=%s -addr=%s -total=%d -pipelining=%t -multiplexing=%t -batch=%t -batch_async=%t -norequest=%t -noresponse=%t -onlycall=%t -clients=%d\n",network,codec,compress,addr,total_calls,pipelining,multiplexing,batch,batch_async,norequest,noresponse,onlycall,clients)
